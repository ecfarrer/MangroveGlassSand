...
library(performance)
library(dada2)
library(sangeranalyseR) #go to GitHub for TMZ file or install(BiocManager)
...
#Useful documentation: https://sangeranalyser.readthedocs.io/en/latest/content/advance_user_SangerAlignment_ab1.html

###### Aligning sequences and calling bases ######
#Originally we tried the M2 (trimmomatic) method with a cutoff quality score of 30 and 10 for the sliding window size. That is what McKenzie did. Her reasoning was that we have lots of low quality bases interspersed in our reads and the M1 (Mott) method (with Phred cutoff 30 or .001 on log scale) will basically truncate the read whereever there is a low quality base. However this M2 method is not very conservative and resulted in a long sequence but with a LOT of ambiguous bases. I also found some concerning things about processing the chromatograms see next comment. [we could potentially use the M2 method with a smaller window size but I didn't try that]

#Notes about how the package is processing the data: this package processes the AB1 files i.e. raw the chromatograms and creates a sequence read from it. Because it is useing its own algorithm for peak calling, the sequence might differ in length than the fasta file that we get from genewiz if it finds another peak or something. This could potentially be fine b/c it should only happen in the crappy region anyway but it means that the base pair numbers might not match up. And it actually seemed to be making some wrong decisions, "seeing" peaks that were teeny tiny and calling that a bp (with a good quality score!), it was a little concerning. When I looked at the chromatogram and compared the genewiz generated reads to those generated by sangeranalyzeR I always aggreed with the genewiz reads and did not like the "decisions" of sangeranalyzeR. Thus I think it is even more important to cut off at bad read quality so that we don't get incorrect bases called
#The reverse read is in normal read order (not reverse complimented), so that matches with the fasta output from genewiz

#Trimmomatic M2 method that I ended up not using. Emily note: we origianlly used 30 and that is what I'll use for the final run. I tried doing 20 for the cutoff score and it qualitatively didn't change the restuls.
# contigs <- SangerAlignment(ABIF_Directory     = "/Users/farrer/Dropbox/EmilyComputerBackup/Documents/LAmarsh/Mangroves/EMILYFARRER05-16-2024_050700_168/Mangrove_30-1030075539_ab1",
#                            inputSource         = "ABIF",
#                            processMethod       = "REGEX",
...
#              outputDir  = "/Users/farrer/Dropbox/EmilyComputerBackup/Documents/LAmarsh/Mangroves/EMILYFARRER05-16-2024_050700_168",
#              compress   = FALSE,
#              selection  = "contigs_unalignment")


#Try with other more conservative (default) method. the forward and reverse reads are trimmed much smaller but can still be aligned, and final read has no ambiguous bases
my_aligned_contigs <- SangerAlignment(ABIF_Directory     = "/Users/farrer/Dropbox/EmilyComputerBackup/Documents/LAmarsh/Mangroves/EMILYFARRER05-16-2024_050700_168/Mangrove_30-1030075539_ab1",
                                inputSource          = "ABIF",
                                processMethod        = "REGEX",
                                REGEX_SuffixForward  = "-ITSF1.ab1$",
                                REGEX_SuffixReverse = "-LR3.ab1$",
...
#Looking at things in the shiny app
launchApp(my_aligned_contigs)

#Write a fasta file. the name was saved as Sanger_contigs_unalignment_NewM1.fasta
...
#First use ITSx https://tools.cifr.ncsu.edu/itsx to separate out the ITS region only. on the website for ITSx upload this file: Sanger_contigs_unalignment_NewM1.fasta   Then download all the output files. The folder is called "itsxT324SZ7G_M1method"
#In that folder take the its_concatenated.fasta file and resave it as its_concatenated_M1method.fasta and put it in the "Stats" folder.
#Then create a csv file in excel with columns called "sequence" and "abundance". under "sequence" copy the ITS sequence for sample 1 and 2 (sample 1 gets one row, sample 2 gets the next row). and for "abundance" write "OTU6" and "OTU7" which is what we've been calling our two samples. save this as its_concatenated_M1method.csv

#Old note about how itsx treats ambiguous bases. I realized that itsx converted all semi-ambiguous bases to N. for example Y was converted to N. So I used the output from itsx to tell me where the ITS region ended and then pulled the actual sequence iwth the semi-ambiguous bases from Kacey's output above and made this file: its_concatenatedquality30ambiguous.csv    In the end there is enough variability within a run on the same csv file, that I can't tell if using the semi-ambiguous bases does anything different
...
myotus<-read.csv("~/Dropbox/EmilyComputerBackup/Documents/LAmarsh/Mangroves/Stats/its_concatenated_M1method.csv",header=T)
rownames(myotus)<-myotus$abundance

taxa<-assignTaxonomy(myotus, unite.ref, multithread = TRUE, minBoot=50, tryRC = TRUE,outputBootstraps=T) # minBoot=70, minBoot should be 70 but I set it lower so that I could see what it was doing. I won't report any taxonomy less than 70
#note from dada2 tutorial: "The original paper recommended a threshold of 50 for sequences of 250nts or less (as these are) but a threshold of 80 generally." Our sequences are slightly over at 293
